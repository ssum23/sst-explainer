...
(function (global) {
  ...
  /* ------------------------------------------------------------------
     ...
     50~100 으로 확대하지 않는다. 견본 A 의 88.6 과 92.4 는 3.8 밖에
  ...
  function factorBar(factor, threshold) {
    ...
    /* 임계선을 마커보다 뒤에 붙인다 — z-index 와 함께 이중으로 보장한다.
       ...
       덮이는 쪽은 기온이 아니라 **일사**다. 트랙 334px · 마커 지름 10px 기준
         일사 88.6  임계 90 에서 1.4%p → 4.7px  → 마커 반지름 5px 안. 덮인다
         기온 92.4  임계 90 에서 2.4%p → 8.0px  → 벗어난다
       덮이면 §9 가 견본 A 를 고른 이유가 화면에서 사라진다. (§5, §14 #16)

       폭을 334px(트랙) 아닌 374px(화면)로 잡으면 일사가 5.2px 가 되어
       "덮인다" 사례가 사라진다. 트랙 폭으로 계산해야 한다. */
  ...
  function groupCard(g, threshold) {
    ...
    /* 그룹 이름 오른쪽에 판정 결과를 붙인다.
       채택 판정은 그룹 단위다 — 견본 A 는 일사(88.6)가 기준에 못 미치는데
       기온(92.4)이 넘어서 그룹으로 채택된다. 카드를 요인마다 나누면
